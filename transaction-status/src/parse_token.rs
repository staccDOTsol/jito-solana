...
pub fn spl_token_instruction(instruction: SplTokenInstruction) -> Instruction {
    Instruction {
        program_id: solana_sdk::pubkey::Pubkey::from_str(&instruction.program_id.to_string()).unwrap(),
        accounts: instruction
            .accounts
            .iter()
            .map(|meta| AccountMeta {
                pubkey: solana_sdk::pubkey::Pubkey::from_str(&meta.pubkey.to_string()).unwrap(),
