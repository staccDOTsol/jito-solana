...
fn get_mint_decimals(bank: &Bank, mint: &Pubkey) -> Option<u8> {
    if solana_sdk::pubkey::Pubkey::from_str(&mint.to_string()).unwrap() == solana_sdk::pubkey::Pubkey::from_str(&spl_token::native_mint::id().to_string()).unwrap() {
...
fn collect_token_balance_from_account(
    ...
    let mint = solana_sdk::pubkey::Pubkey::from_str(&mint.to_string()).unwrap();
